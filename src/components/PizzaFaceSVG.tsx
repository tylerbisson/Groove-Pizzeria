  ...
  return (
    <g transform={`translate(${position.x},${position.y})`} style={{ cursor: 'pointer' }}>
      ...
      {stepAngles.map((angle, i) => {
        ...
        return (
          <line
            ...
            stroke={isSyncSpoke ? 'rgb(120,120,120)' : `rgb(${COLORS.MEDIUM_GREY},${COLORS.MEDIUM_GREY},${COLORS.MEDIUM_GREY})`}
      ...
      {[0, 1, 2].map((ringIdx) => {
        ...
        if (!d) return null;
        return (
          <path key={`shape-${ringIdx}`} d={d + 'Z'}
            fill={shapeFill} stroke={shapeStroke} strokeWidth={1} />
        );
      })}
